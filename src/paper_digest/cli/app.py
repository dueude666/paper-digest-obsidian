...
@app.command("view-paper")
def view_paper_command(
    url_or_id: Annotated[str | None, typer.Argument(help="arXiv URL or arXiv identifier.")] = None,
    title: Annotated[str | None, typer.Option("--title", help="Paper title to search.")] = None,
    topic: Annotated[
    ...
    runtime = build_runtime(verbose=verbose)
    try:
        full_view, result = runtime.workflow.export_full_paper(
            url_or_id=url_or_id,
            title=title,
            topic=topic,
            dry_run=dry_run,
        ...
        if full_view.pdf_asset_relative_path:
            typer.echo(full_view.pdf_asset_relative_path)
        typer.echo(full_view.metadata.title)
    finally:
        runtime.close()
